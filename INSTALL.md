...
audit issues"), and the **GitHub inbox** can pick up Dependabot PRs, security alerts, review
comments and failing checks and turn them into tasks (README → *GitHub inbox*).
...
2. In the dashboard: **Settings → GitHub → Add token**. Give it a name, paste it, press **Test** —
   it reports who the token is and, per project, whether it can read alerts and checks. Save.
